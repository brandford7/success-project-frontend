  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
      <div>
        ...
        <p class="text-gray-600 mt-1">View, edit, and delete all betting tips</p>
      </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
      <div class="bg-white rounded-lg shadow-md p-4">
        ...
        <p class="text-2xl font-bold text-gray-900">{{ tipsStore.pagination.total }}</p>
      </div>
      <div class="bg-white rounded-lg shadow-md p-4">
        <p class="text-sm text-gray-500">Pending</p>
        <p class="text-2xl font-bold text-yellow-600">{{ countByStatus(TipStatus.PENDING) }}</p>
      </div>
      <div class="bg-white rounded-lg shadow-md p-4">
        <p class="text-sm text-gray-500">Won</p>
        <p class="text-2xl font-bold text-green-600">{{ countByStatus(TipStatus.WON) }}</p>
      </div>
      <div class="bg-white rounded-lg shadow-md p-4">
        <p class="text-sm text-gray-500">Lost</p>
        <p class="text-2xl font-bold text-red-600">{{ countByStatus(TipStatus.LOST) }}</p>
      </div>
      <div class="bg-white rounded-lg shadow-md p-4">
        <p class="text-sm text-gray-500">Void</p>
        <p class="text-2xl font-bold text-gray-600">{{ countByStatus(TipStatus.VOID) }}</p>
      </div>
    </div>

    <!-- Loading State -->
    <div v-if="tipsStore.loading" class="text-center py-12 bg-white rounded-lg shadow-md">
    ...
    <div v-else-if="tipsStore.tips.length === 0" class="text-center py-12 bg-white rounded-lg shadow-md">
      ...
      <p class="text-sm text-gray-400 mb-4">Try adjusting your filters or create a new tip</p>
      <NuxtLink
        to="/admin/tips/create"
    ...
    <div v-else class="bg-white rounded-lg shadow-md overflow-hidden">
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="tip in tipsStore.tips" :key="tip.id" class="hover:bg-gray-50">
            <td class="px-6 py-4">
              <div>
            ...
            <td class="px-6 py-4">
              <span :class="getStatusBadgeClass(tip.status)" class="px-3 py-1 rounded-full text-xs font-semibold uppercase">
                {{ tip.status }}
              </span>
            </td>
            <td class="px-6 py-4">
              <span v-if="tip.isVip" class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-amber-100 text-amber-800">
