<template>
  <div class="max-w-7xl mx-auto">
    ...
    <!-- Empty State -->
    <div v-else-if="users.length === 0" class="text-center py-12 bg-white rounded-lg shadow-md">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50">
              <td class="px-6 py-4">
                <div>
                  <p class="font-medium text-gray-900">{{ user.email || user.phoneNumber || 'N/A' }}</p>
              ...
              <td class="px-6 py-4">
                <div v-if="user.isVip">
                  <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-amber-100 text-amber-800 mb-1">
    ...
    >
      ...
      >
        ...
        <p class="text-gray-600 mb-4">
          Grant VIP to: <strong>{{ selectedUser?.email || selectedUser?.phoneNumber }}</strong>
        </p>
...
const confirmGrantVip = async () => {
  ...
  try {
    ...
    alert(error.response?.data?.message || 'Failed to grant VIP')
  } finally {
