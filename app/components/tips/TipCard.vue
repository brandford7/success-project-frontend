<template>
  ...
  >
    <div class="px-6 py-5 flex justify-between items-start">
      <div class="flex-1">
        <div class="flex items-center gap-2 mb-1">
          <h3
            class="text-xl font-black text-gray-900 tracking-tight leading-none"
          >
      ...
      <div class="bg-gray-50 border border-gray-100 rounded-xl p-4">
        <div class="grid grid-cols-3 gap-2 items-center">
          ...
          <div class="px-2 border-r border-gray-200">
            ...
            <p
              class="text-[11px] font-extrabold text-gray-700 uppercase leading-none"
            >
          ...
          <div class="text-right">
            <p
              class="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1 text-right"
            >
              Odds
            </p>
            <p class="text-xl font-black text-blue-700 leading-none">
              {{ tip.odds }}
            </p>
          </div>
        </div>
    ...
    <div v-if="tip.reasoning" class="px-6 mb-5">
      ...
      <p
        class="text-sm text-gray-700 leading-relaxed font-medium italic border-l-4 border-gray-100 pl-4"
      >
        "{{ tip.reasoning }}"
      </p>
    </div>
    ...
    >
      <p
        class="text-[9px] text-blue-600 font-black uppercase tracking-widest mb-1"
      >
        Match Outcome
      </p>
    ...
    <div
      class="px-6 py-4 flex justify-between items-center bg-gray-50/50 border-t border-gray-100"
    >
      <div class="flex items-center gap-5">
        <span
          class="flex items-center gap-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-tight"
        >
          <svg
      ...
      <span
        class="text-[9px] font-black text-gray-300 uppercase tracking-tighter"
      >
    ...
    >
      <div class="flex items-center justify-between">
        <div class="flex gap-5">
          <button
            @click="$emit('edit', tip)"
            class="text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-blue-600 transition-colors"
          >
            Edit Tip
          </button>
          <button
            @click="$emit('delete', tip)"
            class="text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-red-600 transition-colors"
          >
            Remove
          </button>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <select
            v-model="tip.status"
            @change="$emit('quick-status', tip.id, tip.status)"
            class="input text-xs py-1 border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
          >
